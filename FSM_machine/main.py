...
# Создаем класс, наследуемый от StateGroup, для группы состояний нашей FSM
class FSMFillForm(StatesGroup):
    # Создаем экземпляр класса State, последовательно
...
# Этот хендлер будет срабатывать на команду "/start" вне состояний
...
# Этот хендлер будет срабатыввать на команду "/cancel" в состоянии
...
# Этот хендлер будет срабатывать на команду "/cancel" в любых состояниях
...
# Этот хендлер будет срабатывать на команду "/fillform"
# и пререводить бота в состояние ожидания ввода имени
@dp.message(Command(commands='fillform'), StateFilter(default_state))
async def process_fillform_command(message: Message, state: FSMContext):
    await message.answer(text='Пожалуйста введите ваше имя')
    # Устанавливаем состояние ожидания ввода имени
    await state.set_state(FSMFillForm.fill_name)  # Бот зависнет в ожидании ввода имени


# Этот хендлер будет срабатывать, если введено корректное имя
...
# Этот хендлер будет срабатывать, если во время ввода имени
...
# Этот хендлер будет срабатывать, если введен корректный возраст
# и приводить в состояние выбора пола
...
# Этот хендлер будет срабатывать, если во время ввода возраста
# будет введено что-то некорректное
@dp.message(StateFilter(FSMFillForm.fill_age))
...
# Этот хендлер будет срабатывать на нажатие кнопки при
...
# Этот хендлер будет срабатывать, если во время выбора пола
...
# Этот хендлер будет срабатывать, если отправлено фото
...
# Этот хендлер будет срабатывать, если во время отправки фото
# будет введено/отправлено что-то некорректное
@dp.message(StateFilter(FSMFillForm.upload_photo))
...
# Этот хендлер будет срабатывать, если выбрано образование
# и переводить в состояние согасия нолучать новости
...
# Этот хендлер будет срабатывать на выбор получать
# или не получать новости и выводить из машины состояний
@dp.callback_query(StateFilter(FSMFillForm.fill_wish_news),
                   F.data.in_(['yes_news', 'no_news']))
async def process_wish_news_press(callback: CallbackQuery, state: FSMContext):
    # Сохраняем данные о плучении новостей по ключу "wish_news"
...
# Этот хендлер будет срабатывать на отправку команды / showdata
# и отправлять в чат данные анкеты, либо сообщение об отсутствии данных
@dp.message(Command(commands='showdata'), StateFilter(default_state))
async def process_showdata_cammand(message: Message):
    # Отправляем пользователю анкету если она есть "базе данных"
    if message.from_user.id in user_dict:
        ...
        # Если анкеты пользователя нет в "базе данных" - предлагаем заполнить
        await message.answer(
...
async def send_echo(message: Message):
    await message.reply(text='Извините, но я вас не понимаю')
